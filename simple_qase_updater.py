...
# Configure logging
logging.basicConfig(filename='qase_updates_simple.log', level=logging.INFO,
...
for update in updates_to_perform:
    ...
    try:
        ...
        url = f"{BASE_URL}/case/{QASE_PROJECT_CODE}/{case_id}"
        payload = {
            "title": new_title,
            "suite_id": suite_id
        }
        ...
        logging.info(f"Successfully updated case {case_id}. Response: {response.json()}")
    except requests.exceptions.RequestException as e:
        logging.error(f"Error updating case {case_id}: {e}")
    except Exception as e:
        logging.error(f"Unexpected error updating case {case_id}: {e}")
